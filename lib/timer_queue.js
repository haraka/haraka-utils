...
// Sorted delay-queue. Items fire in due-time order via a polling interval.
// O(n) insert (binary search would tighten it; n stays small in practice).
// O(1) fire at the head.
...
class TimerQueue {
  // opts.logger: optional { debug(thisArg, msg) } used only by drain()
  constructor(interval = 1000, opts = {}) {
    this.name = 'haraka-utils/timer_queue'
    this.logger = opts.logger
    this.queue = []
    this.interval_timer = setInterval(() => this.fire(), interval)
    this.interval_timer.unref() // allow process to exit
  }
