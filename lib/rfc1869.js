...
exports.parse = (type, line, strict) => {
  ...
  if (line.length) {
    // Any whitespace still in `line` here is a syntax error: chew already
    // peeled every valid trailing token, so what's left must be the address.
    if (/\s/.test(line)) {
  ...
  // Whitespace in the address would already have thrown above (chewed params
  // can't contain whitespace), so we don't re-check it here.
  if (type === 'mail') {
