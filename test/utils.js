...
exports.uniq = {
    ...
    // OOPS, doesn't work!
    // 'reduces a non-ordered array to unique elements': (test) => {
    //     test.expect(1);
...
exports.encode_qp = {
    ...
    'Very long lines should be broken' : (test) => {
        test.expect(1);
        test.equals(utils.encode_qp("The Quoted-Printable encoding is intended to represent data that largely consists of octets that correspond to printable characters in the ASCII character set."), "The Quoted-Printable encoding is intended to represent data that largely co=\nnsists of octets that correspond to printable characters in the ASCII chara=\ncter set.");
        test.done();
    },
